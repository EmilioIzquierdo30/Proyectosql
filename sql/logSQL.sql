...
BEGIN
    ...
    SELECT 
        ...
        ', Cr�ditos=' + CAST(creditos AS VARCHAR) + 
        ', Regular=' + CASE WHEN is_regular = 1 THEN 'S�' ELSE 'No' END
    FROM inserted
    WHERE NOT EXISTS (
        SELECT 1 
        FROM deleted 
...
-- 3. Repite para las dem�s tablas
...
BEGIN
    ...
    SELECT 
        ...
        ', Cr�ditos=' + CAST(inserted.creditos AS VARCHAR)
    FROM inserted
...
VALUES (20880231, 'Juan P�rez', 1, 40, 'Ingenier�a', 3);
